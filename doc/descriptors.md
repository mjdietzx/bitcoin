...
#### Taproot "decaying" multisig example

For the same "decaying" multisig built as a Taproot output, see [this functional test](/test/functional/wallet_taproot_decaying_multisig_descriptor_psbt.py). Its descriptor pairs a MuSig2 key path with one `multi_a` script-path leaf per decay level: `tr(musig(XPUB1,XPUB2,XPUB3,XPUB4),{and_v(v:after(t1),multi_a(3,XPUB1,XPUB2,XPUB3,XPUB4)),{and_v(v:after(t2),multi_a(2,XPUB1,XPUB2,XPUB3,XPUB4)),and_v(v:after(t3),multi_a(1,XPUB1,XPUB2,XPUB3,XPUB4))}})` (each `XPUB` carries a `/<0;1>/*` multipath suffix, omitted here for brevity). Using separate `multi_a` leaves rather than a single `thresh` keeps `OP_IF` out of the tapscripts.

Whenever all four signers are available, the funds are spent through the **key path** using a single key that aggregates the participants. This is the cheapest and most private spend: it looks like any other single-key output and produces just one signature, though it takes two rounds of communication (each signer first contributes a public nonce, then a partial signature, exchanged as PSBTs). The key path remains available at all times — the wallet does not "decay" into needing fewer signers.

The **script path** is a fallback used only when a key has become inaccessible. Each halvening relaxes that fallback to require one fewer signer (3-of-4, then 2-of-4, then 1-of-4), so a lost key never permanently strands the funds. It is otherwise avoided because revealing the script makes the spend larger and less private.

As in the above [Basic multisig example](#basic-multisig-example), there is a watch-only multisig wallet plus separate single-key signers. Each signer signs the PSBT directly and never imports the multisig descriptor: everything it needs, including the other participants' keys for the key-path rounds, is carried in the PSBT.

[The test](/test/functional/wallet_taproot_decaying_multisig_descriptor_psbt.py) is meant to be documentation as much as it is a functional test, so it is kept as simple and readable as possible.
